...
export default {
  ...
  computed: {
    ...
    vPoints() {
      ...
      return new Array(this.vCount).fill(0).map(
        (x, i) => `${lx(i)},${this.padding} ${cx},${y(i)} ${rx(i)},${this.padding}`,
      ).reverse();
    },
  },
  mounted() {
    this.$watch(
      () => this.isPaused,
      (val) => {
        if (val) this.clearTimeouts(); else this.start();
      },
      { immediate: true },
    );
  },
  ...
  methods: {
    ...
    fadeOut() {
      ...
      const fadeInTimeout = (
        this.fadeOutDelay * (this.vCount - 1)
              + this.fadeDuration + this.fadeInStart
      );
    ...
    fadeIn() {
      ...
      const fadeOutTimeout = (
        this.fadeInDelay * (this.vCount - 1)
              + this.fadeDuration + this.fadeOutStart
      );
