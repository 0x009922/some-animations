...
export default class extends Animation {
    constructor(canvas) {
        ...
        this.geometry.setAttribute(
            'position',
            new THREE.Float32BufferAttribute(new Array(config.count * 3).fill(0), 3),
        );
        this.scene.add(
            new THREE.Points(
                this.geometry,
    ...
    animate(delta, elapsed) {
        ...
        this.dots.forEach((x, i) => {
            ...
            [array[i * 3], array[i * 3 + 1], array[i * 3 + 2]] = pos;
            // array[i * 3] = pos[0];
            // array[i * 3 + 1] = pos[1];
            // array[i * 3 + 2] = pos[2];
        });
    ...
    updateView(elapsed) {
        ...
        const angle = el * 0.1;
        this.camera.position.set(5, Math.sin(angle) * 20, Math.cos(angle) * 20);
        this.camera.lookAt(0, 0, 0);
    }
