...
export default {
    ...
    computed: {
        ...
        dots() {
            ...
            return new Array(this.dotsCount).fill(0).map((m, i) => (i - 0.5) * deltaAngle - Math.PI / 2);
        },
        pairs() {
            const items = [];
            let dot = 0;
        ...
        pairsData() {
            return this.pairs.map((angles, i) => {
                const v = angles.map((a) => new Vector2(Math.cos(a) * this.radius, Math.sin(a) * this.radius));
