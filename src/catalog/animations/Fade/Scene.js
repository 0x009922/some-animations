...
export default class extends CanvasScene {
    ...
    render(delta) {
        ...
        this.fragments.forEach((x) => x.draw(this.context, [width / 2, height / 2]));
    }
}
...
class Fragment {
    ...
    draw(context, center) {
        ...
        let movementProgress = (this.progress - MOVEMENT_THRESHOLD) / (1 - MOVEMENT_THRESHOLD);
        movementProgress **= 0.9;
        if (this.progress <= MOVEMENT_THRESHOLD) {
            movementProgress = 0;
        }

        const startAngle = (-Math.PI * 3) / 4 + DELTA_ANGLE * movementProgress;
        let radius = movementProgress ** 0.8 * (END_RAD - START_RAD) + START_RAD;
        radius = radius < 0 ? 0 : radius;

        for (let i = 0, a = startAngle; i < 4; i++, a += Math.PI / 2) {
            const point = [center[0] + radius * Math.cos(a), center[1] + radius * Math.sin(a)];
            const func = !i ? context.moveTo : context.lineTo;
    ...
    getColor() {
        ...
        const num = Math.round(localProgress * (END_TONE - START_TONE)) + START_TONE;
